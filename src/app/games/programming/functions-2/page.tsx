...
import {
  frogFunctionPond2Levels,
  type BaseCommand,
  type CustomFunction,
  type Direction,
  type FunctionStep,
  type Position,
} from '@/lib/frogFunctionPond2Levels';
...
export default function FrogFunctionPond2() {
  const router = useRouter();
  const { addStars, incrementGamesPlayed, recordAnswer } = useGameStore();
  const levels = frogFunctionPond2Levels;
  ...
  const addCommand = (command: BaseCommand) => {
    if (editingFunction) {
      ...
      setCommands((prev) => [
        ...prev,
        { id: `${Date.now()}-${Math.random()}`, type: 'command', command },
      ]);
    }
  };
  ...
  const cellSize = level.gridSize <= 6 ? 48 : level.gridSize <= 8 ? 40 : 34;
  const nestableFuncs = editingFunction
    ? allFunctions.filter((f) => f.id !== editingFunction.id)
    : allFunctions;

  return (
    <main className="min-h-screen min-h-[100dvh] p-3 sm:p-4 md:p-6 relative overflow-hidden bg-gradient-to-b from-indigo-950 via-violet-950/90 to-slate-950">
      <div
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <motion.div className="text-center mb-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <p className="text-gray-400 text-sm">
            Nest functions inside functions — compress the path under a tight script cap.
          </p>
          <p className="text-indigo-200/80 text-xs mt-1.5 max-w-md mx-auto">
            ...
            <span className="text-amber-300">fly</span>. Nesting depth allowed.
          </p>
          {level.tutorial && (
            <motion.p
              initial={{ opacity: 0 }}
            ...
            <div className="min-h-[150px] max-h-[250px] overflow-y-auto mb-3">
              {commands.length === 0 ? (
                <div className="text-gray-500 text-center py-8 text-xs">
                  Call nested kernels — keep this list short!
                </div>
              ) : (
            ...
            >
              ...
              >
                ...
                <p className="text-violet-300 text-xs text-center mb-4">
                  {allowNesting
                    ? 'You can nest other functions inside this one.'
                    : 'Base hops only.'}
                </p>
